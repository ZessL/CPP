...
int main()
{
	struct peoples{
		...
		bool gender; //Женщина - 0, Мужчина - 1;
		...
		peoples ppl;//Поскольку по условию задания ввод данных с клавиатуры не нужен, то он не реализован
		std::cin>>n;
		char work[30] = "Инженер";
		for(int i = 0; i < n; i++){
			if(gender){
				if((2019-ppl[i].date[2])>65){
					std::cout<<"Имя: "<<ppl[i].Name<<"\n"<<"Фамилия: "<<ppl[i].sName<<"\n"<<"Отчество: "<<ppl[i].thName<<"\n"<<"Пол: "<<ppl[i].gender<<"\n"<<"Должность: "<<ppl[i].work<<"\n"<<"Год рождения: "<<ppl[i].date[3]<<"\n";
					}
				}
				else if(gender == 0){
